...
export default function Home() {
    ...
    const { isConnected, isRecording, transcription, interimTranscription, error: transcriptionError, startRecording, stopRecording, setTranscription, // Get the setter for manual edits
     } = useTranscription();
    // Function to call the generate API
    const handleGenerateContent = async (docType) => {
        if (!transcription) {
    ...
    return (<div className="container mx-auto p-4 md:p-8 min-h-screen flex flex-col gap-8">
     ...
     variant={isRecording ? "destructive" : "default"}>
            ...
            <div className="grid w-full gap-1.5">
              ...
              <Textarea id="transcription-area" rows={10} value={transcription + interimTranscription} // Show final + interim
     onChange={(e) => setTranscription(e.target.value)} // Allow manual edits to the final part
     placeholder="Your transcription will appear here..." className="bg-muted"/>
              {interimTranscription && <p className="text-sm text-muted-foreground italic">Listening...</p>}
            </div>

            {/* Display Errors */}
            {transcriptionError && (<Alert variant="destructive">
           ...
           <CardContent className="flex flex-col gap-4">
            ...
            <div className="grid w-full gap-1.5">
              ...
              <div className="flex flex-wrap gap-2">
                {docTypes.map(type => (<Button key={type} variant={selectedDocType === type ? "default" : "secondary"} onClick={() => handleGenerateContent(type)} // Call API on click
        ...
        >
            ...
            <div className="grid w-full gap-1.5">
              ...
              <div className="flex flex-wrap gap-2">
                {formats.map(format => (<Button key={format} variant={selectedFormat === format ? "default" : "secondary"} onClick={() => setSelectedFormat(format)}>
                    {format}
                  </Button>))}
              </div>
            </div>
            ...
            {/* Download Button */}
            <Button onClick={handleDownload} disabled={!generatedContent || !selectedFormat || isDownloading || isLoading}>
              {/* TODO: Add loading indicator */}
