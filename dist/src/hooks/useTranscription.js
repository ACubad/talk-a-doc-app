...
export function useTranscription() {
    ...
    const startRecording = useCallback(async (languageCode) => {
        ...
        try {
            ...
            ws.current.send('start_stream'); // Send simple string command
            // TODO: Send language code separately if needed by server logic later
            setIsRecording(true);
    ...
    const stopRecording = useCallback(() => {
        ...
        if (ws.current && ws.current.readyState === WebSocket.OPEN) {
            ws.current.send('end_stream'); // Send simple string command
        }
